...
export function TableManagement() {
  ...
  const downloadQR = async (mesa: Mesa) => {
    ...
    await QRCode.toCanvas(canvas, `${window.location.origin}/menu/${negocio?.slug}?mesa=${mesa.numero}`, {
      width: 1024,
      margin: 2,
    })
                      ...
                      {mesa.activa && (
                        <div className="flex items-center gap-1">
                          <div className="bg-white p-1 inline-block rounded">
                            <QRCodeCanvas
                              id={`qr-${mesa.id}`}
                              value={`${baseUrl}/menu/${negocio?.slug}?mesa=${mesa.numero}`}
                              size={48}
                            />
                          </div>
                          <div className="flex flex-col gap-1">
                            <button
                              type="button"
                              onClick={() => window.open(`${baseUrl}/menu/${negocio?.slug}?mesa=${mesa.numero}`, '_blank')}
